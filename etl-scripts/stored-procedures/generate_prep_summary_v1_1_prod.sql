CREATE DEFINER=`fmaiko`@`%` PROCEDURE `generate_prep_summary_v1_2_prod`(IN query_type varchar(50), IN queue_number int, IN queue_size int, IN cycle_size int , IN log boolean)
BEGIN

					select @start := now();
					select @table_version := "flat_prep_summary_v1_2";
                    set @primary_table := "flat_prep_summary_v1_2";
					...
					CREATE TABLE IF NOT EXISTS `flat_prep_summary_v1_2` (
...
-- 							end as prep_start_date_branch,
                        ...
                        replace into flat_prep_summary_v1_2
						(select
							*
						...
						(SELECT
						   concat(week, t1.person_id, week) as elastic_id,
                            ...
                            if( @status = 'active' or @status = 'defaulter', 1, 0) as active_on_prep_this_week,
							...
							left join etl.flat_prep_summary_v1_2 te on (te.person_id = t1.person_id and te.encounter_type = 157 and te.encounter_datetime > t1.encounter_datetime)
